...
- `ReservedCapitalReader` 数据源（[`core/src/service/account/reserved.rs`](../../../crates/quant-pivot-core/src/service/account/reserved.rs)
  + [`repository/src/postgres/quant/reserved_capital.rs`](../../../crates/quant-pivot-repository/src/postgres/quant/reserved_capital.rs)）：
  当前按 intent status 聚合 `sizing_plan.suggested_usd`。重构为读 `quant_capital_allocation`
  的 `locked_usd + allocated_usd`（locked/allocated 状态），trait 不变（`sum_locked` 实现切换到
  `CapitalAllocationRepository::sum_locked`）。**理由**：capital FSM 是资金真相源，避免双账。
- `OrderIntentRepository` 扩展方法 PG impl 落地（05.0 已立签名）：`create_policy_approved` /
  `invalidate` / `find_expired`（`get_for_update` / `mark_admission_rejected` 留 05.4）。
...
impl RuntimeModeGate for DefaultRuntimeModeGate {
    fn intent_policy(&self, mode, recommendation) -> IntentPolicyDecision {
        ...
        // 4. semi_auto → RequiresApproval { required_role: "operator", ttl: config.intent.default_ttl_secs }
...
`ineligibility_reasons` + `approval_required` + `approval_role`）与 `RiskEnvelope.auto_execution_allowed`。
...
pub async fn create_from_recommendation(request, deps) -> QuantResult<OrderIntentInfo> {
    ...
    match policy {
        ...
        RequiresApproval { required_role, ttl } => {
            ...
            deps.tx.create_pending_intent_with_allocation(NewOrderIntent::pending(
                rec, mode, required_role, deps.clock.now() + ttl,
            )).await
        }
...
├── mode_gate.rs        # DefaultRuntimeModeGate（impl）
├── intent_service.rs   # CoreOrderIntentService（impl）+ IntentTransaction（同事务封装）
└── intent_port.rs      # OrderIntentPort（web AppState 端口，仿 QuantReportPort）

crates/quant-pivot-core/src/app/
├── intent_service.rs   # CoreOrderIntentPort wiring（仿 quant_report.rs）
└── periodic_services.rs # + IntentExpirySweep 注册

crates/quant-pivot-web/src/routes/quant_intents.rs   # 新路由模块
crates/quant-pivot-models/src/domain/api/quant_execution.rs  # 新 wire/view DTO
```
...
- **单事务**：intent 状态迁移与 capital FSM 迁移 + op-log **同一 PG 事务**（任一失败全回滚），
  避免"扣了预留但 intent 没建"或反之。
