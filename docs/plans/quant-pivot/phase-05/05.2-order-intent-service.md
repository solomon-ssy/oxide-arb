...
pub async fn create_from_recommendation(request, deps) -> QuantResult<OrderIntentInfo> {
    ...
    match policy {
        ...
        RequiresApproval { approval_ttl } => {
            // ONE txn: intent(PendingApproval) + capital_allocation(Allocated, planned=allocated=sizing.hard_reserved_cash_usd) + op-log
...
- intent 创建（pending/policy）：写 `quant_capital_allocation`（state=`Allocated`，
  `planned_usd=allocated_usd=sizing.hard_reserved_cash_usd`），**与 intent 同事务**。
