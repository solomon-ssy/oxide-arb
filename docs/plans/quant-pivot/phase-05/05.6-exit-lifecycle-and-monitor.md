...
- **admission `#20` 真实化**：`ExitMonitorHealthHandle`(ArcSwap) 由 worker 每次成功扫描发布心跳；
  builder 读 `is_ready(now)`（spawned ∧ now−last_scan ≤ 2×monitor_secs）；entry-authorization preflight `exit_monitor_healthy`
...
| 延后能力 | 详细设计 | 05.6 本期替代 |
|---|---|---|
| 多副本 exit/recon/dispatch leader 选举 | Deferred；不存在 active `phase-08/README.md` 实施入口 | 当前只支持文档冻结的单实例 owner |
| Trailing stop 高频 peak 跟踪 | Deferred；未经新设计不恢复旧 Phase 8 方案 | 当前以 canonical exit-monitor contract 为准 |
