...
- **`HoldToResolution`**：抑制 take-profit/time/partial/opportunistic 等"获利/超时"档，仅保留保护性档
  （emergency / data-stale / market-abnormal / stop-loss / signal-invalidation）；结算后 `Settled` 与
  `AutoRedeem` 链上赎回延后（§11）。
...
|---|---|---|
| `SettlementPolicy::AutoRedeem` 链上 CTF 赎回 | **[`05.10-auto-redeem-settlement.md`](05.10-auto-redeem-settlement.md)**（alloy `redeemPositions`、`TaskId::SettlementRedeem`、per-lot payout 落账） | 独立 on-chain venue 集成；05.6 仅 `HoldToResolution` → `Settled` 标记 |
