...
- `QuantRuntimeControl::switch_quant_mode`（[`runtime_control.rs`](../../../crates/quant-pivot-core/src/governance/runtime_control.rs) §42–57）：
  ...
  `ModePreflightFailed`，**不**持久化、**不** store（fail closed）。
...
```rust
// core/src/execution/mode_gate.rs
...
pub trait ModePreflight: Send + Sync {
    async fn run(&self, target: QuantRuntimeMode) -> Result<PreflightReport, ExecutionError>;
}

pub struct PreflightReport {
    pub target: QuantRuntimeMode,
    pub checks: Vec<PreflightCheck>,        // 每项 name + passed + detail
...
| JWT secret strong | deploy JWT 强度（复用 `validate_for_quant_mode`，[`config/mod.rs`](../../../crates/quant-pivot-models/src/config/mod.rs) §141–159） |
...
```rust
// core/src/execution/kill_switch.rs（或 governance/kill_switch.rs）
...
pub trait KillSwitchService: Send + Sync {
    ...
    async fn set(&self, target: KillSwitchState, actor: &str, reason: &str)
        -> QuantResult<KillSwitchView>;     // 持久化 + store + 审计 + metric
    /// 门禁查询（admission / dispatcher / exit monitor 消费）。
    fn allows_new_entry(&self) -> bool;
    fn allows_auto_exit(&self) -> bool;
    fn requires_emergency_exit(&self) -> bool;
}
```
...
├── execution/
│   ├── mode_gate.rs        # ModeTransitionGate + DefaultModeTransitionGate（矩阵）
│   ├── mode_preflight.rs   # ModePreflight + DefaultModePreflight（检查清单）
│   └── kill_switch.rs      # KillSwitchHandle + KillSwitchService + KillSwitchServiceImpl
└── governance/runtime_control.rs  # switch_quant_mode 接入 gate+preflight；system_status 读真实态
...
`switch_quant_mode` 重构后流程：

```rust
let from = self.runtime_mode.current();
if from == target { return Ok(no_op_report(from)); }
self.transition_gate.check(from, target)?;                  // 矩阵
if target.requires_preflight() {                            // 升级
    let report = self.preflight.run(target).await?;
    if !report.passed { return Err(ExecutionError::ModePreflightFailed(report.summary()).into()); }
}
...
- **错误分层**：`ExecutionError::{ModeTransitionForbidden, ModePreflightFailed}`；web 映射 409。
