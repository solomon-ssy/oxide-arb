...
| `WeightedFactorRuntime`（generic）+ `DefaultModelRuntimeFactory` | ✅（3.4） |
...
/// PIT query over stored domain observations. Mirrors `PitQueryEngine` (3.5).
...
> `<= as_of - source_delay` 最新值，零 DB），与 3.5 `MaterializedPitEngine` 同范式，保证在线/离线
> byte-identical。
...
（对齐 3.5 `QuantFactReadRepository` 范式：domain-typed bind、显式稳定排序、`event_time <= as_of - source_delay`）。
...
- 缺失 ⇒ `raw_value: None` / `confidence: 0`；`MissingFactorPolicy::ZeroWeight` 默认。
...
（poll 是特征不是 label）。PIT：以民调发布日截断（民调发布有天级滞后，`source_delay` 须足够大）。
...
- **runtime `domain` 段**：per-`DomainFamily` enable / `source_delay_secs` / refresh 策略 / `cross_check` 阈值。
