...
- classical ML：`ClassicalModelAdapter` over smartcore（仅 `LogisticRegression` payout classifier）+ `bincode` 序列化
...
**唯一 implemented kind（smartcore 0.5）：** `LogisticRegression`。它只接受 winner-take-all binary rows，
输出未校准的 full-payout probability，并在独立 calibration/scenario contract 中与 split-payout mass 合成为
完整 payout distribution。该 family 在完成校准前只能 research/shadow。所有 forward-return regressor、
tree ensemble、penalized-linear regression 及其 runtime/artifact enum 已 clean-break 删除。
...
- `ml-classical`：`smartcore`（仅 payout `LogisticRegression`）。
