...
```rust
// 对齐 03.3 精炼契约：domain factor 复用 FactorComputer 的 compute_raw（raw/normalize 分离），
// 仅以 DomainFamily 标记路由；归一化仍由 FactorEngine 统一负责。
pub trait DomainFactorComputer: FactorComputer {
    fn family(&self) -> DomainFamily;
}
// 缺失 domain 输入 ⇒ RawFactor{ raw_value: None, confidence: 0 }（绝非静默 0）；
// 置信度折减/中性 imputation 由 3.4 模型层按 artifact 声明施加。
```
...
- 03.3 已交付：`DomainFactorComputer` trait + 空的 by-category `DomainFactorRegistry`
  （`for_category` 返回该垂直已注册因子，未接外部源时为空，不阻塞 generic 报告）
...
> **`required_features` 流**：3.4 经 `QuantModelRuntime::required_features()` 暴露
> artifact 字段；**Phase 04** `ReportBuilder` 注入 03.1
> `MarketSelectionBuildRequest.model_requirements`（3.4 单测/e2e 仍可用空需求）。
> category-specific 路由的按 market 可变维度问题仍按 03.2 §3 与真实 domain 源接入一并设计。
...
| `category_specific_model_routes_sports` | Post–domain 源 | routing（[`03.4` §10](03.4-model-runtime-weighted-scorer.md)） |
| `classical_sports_rf_shadow_infer` | 3.6 | smartcore |
| `vertical_backtest_category_breakdown` | 3.6 | metrics |
| `onnx_geopolitics_embedding_infer` | Phase 06+ | ort |

跨 phase defer 总表：[`README.md`](README.md) §6。
...
| 2025-06 | 3.4 落地后：`required_features` 全链路编排 → Phase 04；category-specific model / ONNX → defer 表（README §6） |
