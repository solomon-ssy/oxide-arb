...
> **历史版本说明：**11.6 完成时基线是 runtime v10 / feature v6 / dataset+model v2；当前版本见
> [Phase 11 权威版本账本](README.md#权威-boot-版本账本当前)。下文 v4/v5 只记录 11.2.1/11.2.2 历史波次。
> 时间、缺失、category route 与 parity 以 [11.6](11.6-training-serving-parity-and-no-silent-zero.md) 为准。
>
...
>   + 当时的历史 schema 波次 + 治理页）。**无外部数据源**。关闭审计点 **#4 前半**。
...
>   历史 runtime-config **v5** / `feature_schema_version` **5** 波次）。**已落地**。关闭审计点 **#4 后半**，
>   最终闭合 #4。
...
```mermaid
flowchart LR
  P111["11.1 通用因子地基"] --> P1121["11.2.1 平台内结构 (历史 v4 波次)"]
  P1121 --> P1122["11.2.2 crypto 外部垂直 (历史 v5 波次 / 两层向量) ✓"]
