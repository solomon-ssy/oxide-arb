...
│   └── domain/ch_source.rs      # ChDomainPitSource
├── prefetch/
...
crates/quant-pivot-research/src/
├── domain/slice.rs              # build_domain_slice_inputs (纯 PIT 组装)
└── pit/trade_tape_window.rs
```
...
| **Chainlink Data Streams** settlement oracle ingest + basis PIT | `ResolutionOracle::ChainlinkDataStreams` 已建模；oracle 价 ingest、`basis_vs_resolution_source` 全量 PIT 与 cross-check 告警 → 独立 follow-up（非 Tier 2）。 |
