...
>
> **11.6 横切契约覆盖（当前真值）**：本文保留 crypto 垂直的来源/linkage 设计，但时间、缺失和模型路由
> 必须遵守 [`11.6`](11.6-training-serving-parity-and-no-silent-zero.md)：入口是
> `DecisionBoundary(decision_at, knowledge_cutoff, per_source_cutoffs)`，下游不得再次减 lag；domain evidence
> 同时受 source cutoff 与 `available_at <= decision_at` 约束；缺失由 `FeatureCell` / `ModelInputContract`
> 表达，category model 任一故障整轮失败，禁止 category→generic 或 ZeroWeight fallback。当前唯一可激活
> contract 是 boot schema `1`；下文旧数字只记录被取代的实施波次。
>
...
> re-export、不复用旧命名**。这些能力直接进入 boot Feature/Domain profile schema `1`。
>
...
| favorite_longshot `FavoriteLongshotBiasTableId` | 11.2.1 独立 artifact | fresh bootstrap 直接收敛到 `CalibrationArtifactId` 治理，不保留并行 ID。 |
...
- Boot contract 直接包含两层向量。**结构因子的特征仍归 generic slice**（平台内可算，恒存在），domain slice 只装外部垂直特征。
...
/// Assemble the PIT domain slice for one `(category, market, decision_at)` — the
...
pub fn build_domain_slice_inputs(
    category: MarketCategory, linkages: &[MarketLinkage], boundary: &DecisionBoundary,
    domain: &DomainConfig, observations: &HashMap<DomainInstrumentKey, Vec<DomainObservation>>,
) -> QuantResult<Option<DomainSliceInputs>>; // effective/publish <= source cutoff; available_at <= decision_at
```
...
- `FeatureAvailabilityOracle` 重建 domain 可用性：category→family 有已接源 ∧ 存在 `Resolved` linkage ∧ 源在
  `DecisionBoundary` 下有 PIT 数据。
- **11.6 fail-closed 路由**：domain 缺失由 feature state 与模型 input requiredness 决定；required input 缺失
  直接拒绝 market。category-specific route 的加载、scope 或 inference 任一失败都使整轮报告失败，禁止回落
  `GenericWeighted`，也禁止以 ZeroWeight 把缺证据伪造成有效输入。
...
- leakage：domain 证据的 effective/publish time 不晚于 frozen source cutoff，且 `available_at <= decision_at`。
...
- **ClickHouse**：`quant_domain_observation`（长格式，5 垂直共用；PIT 读 `observations_at`/`observations_between`
  同时约束 `event_time/publish_time <= source_cutoff` 与 `ingestion_time <= decision_at`，并使用稳定排序）。
...
## 5. Deploy binding 与 immutable profile path

旧统一 Runtime Config schema 已删除；以下语义拆入 boot `FeatureProfileArtifact`、`DomainProfileArtifact`
与 provider Deploy binding，均拒绝 unknown fields。

```text
# immutable boot profiles
...
domain.crypto.availability_lag_secs          = 5
...
- **Chainlink 现行契约（由 11.7.1 破坏式取代）**：Polygon `AggregatorV3` Data Feeds、round walker、`DomainMetric::OraclePrice` 与旧 cursor/config 已删除。短周期 Chainlink-settled 市场只绑定官方 **Data Streams V3** signed report；Binance 仅是 feature/cross-check source，不能替代 live event 或 resolution source。完整来源、gap recovery、reference report 与激活门禁以 [11.7.1](11.7.1-composable-entry-event-triggers.md) 为唯一现行设计。
...
- `domain_observation_pit_honors_decision_boundary_once`（对 `build_domain_slice_inputs` 断言，非双引擎镜像；
