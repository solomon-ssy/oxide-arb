...
> 最后更新：2026-07-18
>
> 版本所有权：Runtime Config **v17 → v18**；Feature Schema **v6 → v7**；Dataset Artifact
> **v5 → v6**；Model Artifact **v5 → v6**；Trade Policy v6、Evidence Bundle v3 保持不变。
>
...
| 领域 | 当前事实 | 结论 |
|---|---|---|
| Runtime | 当前代码已是 v17，且空 `FeedbackConfig` 已删除 | 旧文档的 v16→v17 不再成立；本期必须显式 v17→v18 |
...
flowchart LR
    ...
    B --> D["Sealed Dataset v6"]
    D --> M["Model Artifact v6"]
...
7. **零兼容**：v17/v5 artifact 仅可审计读取；runtime 不提供旧 parser、双读、alias 或 re-export。
...
| 合同 | Before | After | 原因 |
|---|---:|---:|---|
| Runtime Config | 17 | 18 | 新增 feedback 运维字段；方法论仍在 immutable profile |
| Feature Schema | 6 | 7 | 通用 Weather 变量、factor-bundle binding、双垂直 serving contract |
| Dataset Artifact | 5 | 6 | 绑定 factor bundle、cohort coverage ledger 与 capability registry hash |
| Model Artifact | 5 | 6 | 绑定 factor bundle ID/hash、profile feedback policy 与 evaluation window |
| Trade Policy | 6 | 6 | 本期不改变交易权限或策略语义 |
| Evidence Bundle | 3 | 3 | 扩展引用，不改变现有 envelope wire |

Runtime v18 只允许以下运维字段：
...
feedback.auto_publish_categories = [crypto, weather]
```

禁止在 Runtime 放入 PSI/KS、minimum improvement、CPCV/DSR/PBO、shadow duration、effect size 等研究
阈值；它们属于 `ResearchProfileSpec.feedback_policy`，随 profile hash 冻结。
...
Dataset v6 与 Model v6 都必须绑定 bundle ID/hash。Active 与 Shadow model 分别加载自己的 bundle；删除
...
| 删除对象 | 替代 |
|---|---|
| 06.5 旧设计与 11.9 v16/v17/空 FeedbackConfig 描述 | 本文 v18 契约 |
...
| 日期 | Item | 结果 | 证据 |
|---|---|---|---|
| 2026-07-18 | W0-01 | PASS | 本文完成最终设计与 Implementation/Evidence Ledger；旧目标版本已更正为当前 v17→v18 |
| 2026-07-18 | W0-02 | PASS | `cargo test -p quant-pivot-models runtime_config --lib`：42 passed、0 failed、1 ignored；Runtime v18、feedback 运维边界、schema leaf/locale/validation 全部通过 |
...
| 2026-07-18 | W1-10 / W1-12 | FAIL（import-style gate，修复中） | 第四轮 targeted all-target Clippy 与精确 migration Docker test 均 PASS；随后 import-style gate 在本期新增/改动路径发现 16 个 body deep-path 与 1 个重复 `quant_pivot_research` root：evidence bootstrap 的 `RuntimeConfigStore/MarketCategory/DomainSourcesConfig`、weather fact `mem::replace`、xtask runtime repo、weather worker import tree。按 preamble-only/one-tree-per-root 规则显式导入并缩短 body path，不运行全仓机械 `--fix`、不改无关文件；修复后重跑全部五项 lint 与 `git diff --check` |
...
| 日期 | 决策 | 原因 | 影响 |
|---|---|---|---|
| 2026-07-18 | Runtime 使用 v18，不沿用旧文档 v16→v17 | 当前代码已是 v17 | 所有旧 runtime 文档必须同步 |
