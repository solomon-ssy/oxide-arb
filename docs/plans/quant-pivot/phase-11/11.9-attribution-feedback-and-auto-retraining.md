...
> 状态：**设计冻结，尚未实施**。本 phase 以 11.8 完成后的 runtime config **v16**、feature schema **v6**、
> dataset/model artifact v5、Trade Policy v6、Evidence Bundle v3、`FeedbackConfig {}`；
> `json_schema/fields.rs` 的 empty-closed-object 临时处理仍按设计保留。只有 11.9 真正加入首批 feedback
> 字段时才升级 runtime config v16→v17 并删除该临时代码；不得把未来 wire contract 写成当前状态，也不得
> 顺带虚构 feature v7 或 artifact v7。
>
...
> - 前置 11.8 把 runtime config 升级为 **v16**；本子phase 向 `feedback` 空段加入第一批真实字段时升级为
>   **v17**。不得静默改写 v16，也不得回退到旧版本迁移叙述。
> - 11.8 交付 `RecommendationAttributionOutcome::SupersededUnfilled`；它是 censored observation，
>   coverage/dataset refresh 必须单独计数并排除，禁止把 report replacement 当作模型负收益。
...
>   `is_empty_closed_object` 辅助函数及关联单测。v17 再无空 closed object 段，
>   遗留即死代码(见 §2 删除清单)。
...
| `json_schema/fields.rs` `is_empty_closed_object` 早退 + 辅助函数(11.0 临时) | `feedback` 仍空时不产 UI leaf | **删除(11.9 Blocker)**:本子phase 向 `FeedbackConfig` 加字段后,v17 再无空 closed object 段;须删除 `walk_schema` 早退块(约 135–141 行)、`is_empty_closed_object`(约 164–175 行)及关联单测;`research`/`feedback` 改由 properties 分支自然产 leaf。 |
...
`feedback` 段（**未来实施目标，不是当前可用配置**；11.8 完成 v16 后，本子phase
**v16 → v17**，11.0 预留的空骨架在这里第一次获得真实字段）：
...
该 bump 只描述 runtime-config wire `v16 → v17`。11.9 直接复用 feature v6、dataset/model artifact v5、
Trade Policy v6、Evidence v3、
PIT facts 与 parity latch；除非届时另有经过迁移与 loader fail-closed 设计的真实序列化语义变更，不得在
本期文档或实现中擅自宣称 feature v7 / artifact v7。
...
- 不得重新引入 11.8 已删除的 `tokio-cron-scheduler`；feedback 调度复用届时现有的 durable research-job
  queue/worker 账本。11.8 的 report coordinator 只拥有 report runs，不得被泛化成无边界通用 scheduler。
...
- 11.6/11.7.2 尚无真实维护窗口、Source Slice restore、ReportOnly shadow 或最新环境门禁证据，却激活
  v17 feedback worker、注册可激活 profile 或分配跨 profile 资金。
...
- 当前 11.6 的 W4 代码/页面目标面已有实现，但最终合并态全量门禁尚待重跑；W5 运维切换与
  ...
  本文件所有 v17 key、worker、表和 API 均仍是待实现契约，不能作为当前生产能力或迁移已完成的证据。
- 回指审计点:#19、#21 在此闭合。
