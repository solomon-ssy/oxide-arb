...
> 治理页与可视化。**不含任何外部数据源**（crypto/Binance 归 11.2.2）。
>
> **历史版本说明：**本文的旧版本数字是早期设计波次记录，已被 boot schema `1` 取代；当前版本见
...
> 填充时再改）。本期不再执行历史版本 bump；结构直接进入 boot feature/config contract。
>
...
- **强反转性 / 参与者集中**：大幅波动后 24–48h 显著回补；真实 participant concentration 依赖
  finalized exchange participant facts（11.2.1.1），本 11.2.1 只保留 book-derived churn 信号，不把它命名为 maker concentration。
...
| favorite_longshot 偏差表 | 本期 11.2.1 自有 artifact | **唯一终态**：fresh bootstrap 直接由 `CalibrationArtifactId` 治理（§3.5 + 11.3 §3.4），不保留平行 ID 或转换路径。 |
...
- `micro.book_churn` + depth 变化（book churn regime，`features.structural.book_churn_window_secs`；真实
  participant concentration 由 11.2.1.1 的 finalized execution participant fact/feature 提供）。
...
Boot `FeatureProfileArtifact` 的 schema builder 在 `FeatureFamily::Structural` 启用时追加上述 spec。
...
`ic_significant=false`/桶 `bias=None` → 因子 disabled（confidence=0）。`bias_table_ref` 在唯一
fresh bootstrap 中直接统一到 `CalibrationArtifactId`。
...
## 5. Deploy 与 immutable profile path

旧统一 Runtime Config bump 已删除；以下语义直接进入 boot `FeatureProfileArtifact` 与
`ScoringProfileArtifact`，拒绝 unknown fields。
...
- boot profile 未通过 typed parse、semantic validation 与 activation preflight 双门。
