...
/// Revision of the stable, typed XGBoost model export contract.
pub const XG_TYPED_EXPORT_REVISION: u32 = 1;

/// A repository-independent node in a fitted XGBoost tree.
///
/// This read-only representation exposes the exact values used by prediction
/// without exposing the estimator's private serialized document layout.
#[derive(Clone, Debug, PartialEq)]
pub enum XGTreeNodeExport {
    /// A terminal prediction correction.
    Leaf {
        /// The unscaled leaf value. Callers apply the model learning rate.
        value: f64,
    },
    /// A binary split. Values less than or equal to `threshold` take `left`.
    Split {
        /// Zero-based input feature index.
        feature_index: usize,
        /// Inclusive threshold for the left branch.
        threshold: f64,
        /// Left child for values less than or equal to the threshold.
        left: Box<Self>,
        /// Right child for values greater than the threshold.
        right: Box<Self>,
    },
}

/// Stable, typed read-only export of a fitted [`XGRegressor`].
#[derive(Clone, Debug, PartialEq)]
pub struct XGModelExport {
    /// Export contract revision.
    pub revision: u32,
    /// Initial prediction before tree corrections.
    pub base_score: f64,
    /// Multiplier applied to every tree correction.
    pub learning_rate: f64,
    /// Trees in prediction order.
    pub trees: Vec<XGTreeNodeExport>,
}
    ...
    TreeRegressor<TX, TY, X, Y>
{
    ...
    fn export(&self) -> Result<XGTreeNodeExport, Failed> {
        match (&self.left, &self.right) {
            (None, None) => Ok(XGTreeNodeExport::Leaf { value: self.value }),
            (Some(left), Some(right)) => Ok(XGTreeNodeExport::Split {
                feature_index: self.split_feature_idx,
                threshold: self.threshold,
                left: Box::new(left.export()?),
                right: Box::new(right.export()?),
            }),
            _ => Err(Failed::invalid_state(
                "XGBoost tree contains a node with exactly one child",
            )),
        }
    }
}
...
impl<TX: Number + PartialOrd, TY: Number, X: Array2<TX>, Y: Array1<TY>> XGRegressor<TX, TY, X, Y> {
    ...
    /// Export the fitted model through the stable typed tree contract.
    ///
    /// The export is read-only and preserves tree order, branch semantics,
    /// thresholds, leaf corrections, learning rate, and base score exactly as
    /// used by [`Self::predict`].
    pub fn export_model(&self) -> Result<XGModelExport, Failed> {
        let parameters = self
            .parameters
            .as_ref()
            .ok_or_else(|| Failed::invalid_state("XGBoost model is not fitted"))?;
        let regressors = self
            .regressors
            .as_ref()
            .ok_or_else(|| Failed::invalid_state("XGBoost model is not fitted"))?;
        let trees = regressors
            .iter()
            .map(TreeRegressor::export)
            .collect::<Result<Vec<_>, _>>()?;
        Ok(XGModelExport {
            revision: XG_TYPED_EXPORT_REVISION,
