...
use quant_pivot_models::{
    ...
    domain::{
        LifecycleSchemaVerificationPort, NewProductionBaseline, NewProductionEvidence,
...
impl DisposableDirectory {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!("quant-pivot-w9-{}", Uuid::now_v7()));
...
async fn clean_boot_recovers_each_system_stage_and_restores_backups() {
    ...
    let clickhouse = testcontainers::GenericImage::new("clickhouse/clickhouse-server", "26.5")
    ...
    fs::set_permissions(&bootstrap_password_file, fs::Permissions::from_mode(0o600))
...
async fn run_xtask(args: Vec<String>, bootstrap_password_file: Option<&Path>) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_quant-pivot-xtask"));
...
async fn configure_disposable_postgres_user(container: &testcontainers::ContainerAsync<Postgres>) {
    container
        .exec(
            ExecCommand::new([
                "psql",
...
async fn set_postgres_create_database_allowed(
...
async fn configure_disposable_redis_user(container: &testcontainers::ContainerAsync<GenericImage>) {
...
async fn set_redis_unlink_allowed(
...
async fn record_disposable_evidence(
    ...
    bundle: &quant_pivot_models::runtime_config::ActivePolicyBundle,
) -> quant_pivot_models::domain::ProductionEvidenceInfo {
    repository
...
fn write_read_only_evidence(
    ...
    let uri = url::Url::from_file_path(&path).expect("disposable evidence file URL");
    (
