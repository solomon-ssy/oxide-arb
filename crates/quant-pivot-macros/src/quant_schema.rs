//! `#[quant_schema]` — iden derive + schema catalog registration.
...
pub fn expand(args: TokenStream, input: TokenStream) -> Result<TokenStream> {
    ...
    let Data::Enum(enum_data) = &input.data else {
        return Err(Error::new_spanned(
            &input,
            "quant_schema can only be applied to iden enums",
        ));
    };
    ...
    if !has_table {
        return Err(Error::new_spanned(
            name,
            "quant_schema iden enums must contain a `Table` variant",
        ));
    }
    ...
    let table_fn = format_ident!("__quant_schema_{lower}_table_name");
    let triggers_fn = format_ident!("__quant_schema_{lower}_auto_triggers");
...
fn parse_lifecycle(args: TokenStream) -> Result<(TokenStream, bool)> {
    ...
    if !name_value.path.is_ident("lifecycle") {
        return Err(Error::new_spanned(
            name_value.path,
            "unsupported quant_schema argument",
        ));
    }
