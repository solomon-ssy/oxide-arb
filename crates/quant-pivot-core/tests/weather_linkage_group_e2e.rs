//! Weather linkage decision-group orchestration against real `PostgreSQL`.
...
fn hash<T: serde::Serialize>(value: &T) -> ContentHash {
...
async fn seed_group(db: &sea_orm::DatabaseConnection, event_id: &str, members: &[(&str, &str)]) {
...
fn service(db: &sea_orm::DatabaseConnection) -> LinkageResolverService {
    LinkageResolverService::new(
        ...
        station_profiles(),
        &quant_pivot_models::config::WeatherVerticalBindingsConfig::default(),
    )
...
async fn single_sibling_request_validates_and_atomically_appends_the_complete_group() {
