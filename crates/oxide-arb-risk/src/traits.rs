...
pub struct RiskMetricsSnapshot {
    ...
    /// Seconds since the target market's YES/NO book tokens both received WS data.
    pub market_ws_disconnect_secs: u64,
...
pub trait RiskMetrics: Send + Sync + 'static {
    ...
    fn snapshot_for(&self, market_id: &MarketId) -> RiskMetricsSnapshot {
        RiskMetricsSnapshot {
            ...
            market_ws_disconnect_secs: self.ws_disconnect_secs(),
