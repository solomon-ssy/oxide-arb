...
impl CapitalAllocationRepository for PgCapitalAllocationRepository {
    ...
    async fn sum_reserved_usd(&self) -> Result<Usd, StorageError> {
        Self::sum_reserved_usd(&self.db).await
    }
...
impl PgCapitalAllocationRepository {
    ...
    pub async fn load_capital(
        ...
        Entity::find()
            ...
            .await
            .map_err(StorageError::from)?
            .ok_or_else(|| StorageError::not_found(QUANT_CAPITAL_ALLOCATION, intent_id))
    }
}
...
impl PgCapitalAllocationRepository {
    ...
    pub async fn release_capital(
        ...
        let cap = Self::load_capital(db, intent_id).await?;
...
impl PgCapitalAllocationRepository {
    ...
    pub async fn settle_capital(
        ...
        let ok = validate_non_negative(cap.allocated_usd, locked_usd, spent_usd, released_usd)
            .is_ok()
            && capital_invariant_ok(
                cap.planned_usd,
                cap.allocated_usd,
