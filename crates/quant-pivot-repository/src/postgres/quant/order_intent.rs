...
impl OrderIntentRepository for PgOrderIntentRepository {
    async fn create_with_allocation(
        ...
        validate_intent_allocation(&intent, &allocation)?;
        ...
        PgRuntimeControlRepository::require_entry(&txn).await?;
        let active_policy_snapshot_id = Self::load_policy_snapshot(&txn).await?;
        if active_policy_snapshot_id.as_ref() != Some(&intent.decision_policy_snapshot_id) {
            return Err(StorageError::state_conflict(
                QUANT_ORDER_INTENT,
                None::<&OrderIntentId>,
                "recommendation decision policy is no longer the active policy snapshot",
            ));
        }
        if Self::find_blocking_intent(&txn, &intent.recommendation_id)
            .await?
            .is_some()
        {
    ...
    async fn approve(
        ...
        let (rec, report) =
            Self::load_recommendation_with_report(&txn, &row.recommendation_id).await?;
        let active_policy_snapshot_id = Self::load_policy_snapshot(&txn).await?;
        let kill_switch_allows_entry = PgRuntimeControlRepository::entry_allowed(&txn).await?;

        let invalidation = if row.expires_at <= now {
            Some(ApprovalInvalidation::IntentExpired)
        } else {
            match active_policy_snapshot_id.as_ref() {
                Some(active_snapshot_id) => rec.approval_invalidation(
                    &report,
        ...
        if let Some(reason) = invalidation {
            let intent_model =
                Self::transition_invalidated(&txn, intent_id, row, reason, now, false).await?;
            txn.commit().await.map_err(StorageError::from)?;
        ...
        let intent_model = Self::apply_approval(
            &txn,
            intent_id,
            row,
            approval,
    ...
    async fn reject(
        ...
        .await?;
        Self::insert_terminal_operation_log(&txn, operation_log, &before_info, &intent_model)
            .await?;
        txn.commit().await.map_err(StorageError::from)?;
        Ok(intent_model.into())
    }
    ...
    async fn expire(
        ...
        .await?;
        PgCapitalAllocationRepository::release_capital(&txn, intent_id, "expired".to_owned())
            .await?;
    ...
    async fn invalidate(
        ...
        let before_info: OrderIntentInfo = row.clone().into();
        let intent_model =
            Self::transition_invalidated(&txn, intent_id, row, reason, invalidated_at, true)
                .await?;
        let after_info: OrderIntentInfo = intent_model.clone().into();
        let operation_log =
            state_hash::apply_transition_hashes(operation_log, &before_info, &after_info)?;
    ...
    async fn find_by_ids(
        ...
        find_id_chunks::<QuantOrderIntentEntity, _, _>(&self.db, intent_ids, Column::OrderIntentId)
            .await
    ...
    async fn find_active_by_recommendation(
        ...
        Self::find_blocking_intent(&self.db, recommendation_id)
            .await
            .map(|row| row.map(Into::into))
    }
...
impl PgOrderIntentRepository {
    async fn apply_approval(
        ...
        if let Some(new_allocated) = allocated_override {
            let cap = PgCapitalAllocationRepository::load_capital(db, intent_id).await?;
...
impl PgOrderIntentRepository {
    ...
    async fn transition_invalidated(
        ...
        PgCapitalAllocationRepository::release_capital(
            db,
            intent_id,
            format!("invalidated: {}", reason.as_str()),
        )
        .await?;
        Ok(intent_model)
    }
...
impl PgOrderIntentRepository {
    async fn insert_terminal_operation_log(
        ...
        let operation_log =
            state_hash::apply_transition_hashes(operation_log, before, &after_info)?;
        OperationLogEntity::insert(operation_log.into_active_model())
            .exec(db)
            .await
            .map_err(StorageError::from)?;
...
impl PgOrderIntentRepository {
    ...
    pub async fn invalidate_pre_submission(
        db: &impl ConnectionTrait,
        recommendation_id: &RecommendationId,
        reason: ApprovalInvalidation,
        occurred_at: DateTime<Utc>,
...
impl PgOrderIntentRepository {
    pub async fn lock_terminal_graph(
        ...
        let probe = Self::load_intent(db, intent_id).await?;
        let recommendation = Entity::find_by_id(probe.recommendation_id)
            ...
            .map_err(StorageError::from)?
            .ok_or_else(|| {
                StorageError::not_found(QUANT_RECOMMENDATION, probe.recommendation_id)
            })?;
        let intent = Self::load_intent_for_update(db, intent_id).await?;
        ...
        let condition = QuantEntryConditionInstanceEntity::find_by_id(intent.condition_instance_id)
            ...
            .map_err(StorageError::from)?
            .ok_or_else(|| {
                StorageError::not_found(ENTRY_CONDITION_ENTITY, intent.condition_instance_id)
            })?;
        ...
        let _capital = PgCapitalAllocationRepository::load_capital(db, intent_id).await?;
        Ok(intent)
    }
}
