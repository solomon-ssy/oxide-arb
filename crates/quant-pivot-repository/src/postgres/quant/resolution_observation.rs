...
use quant_pivot_models::{
    domain::{
        ...
        quant::{
            NewResolutionObservationInbox, ResolutionObservationInboxInfo,
            ResolutionObservationProjectionInfo, ResolutionProjectionBarrier,
...
use crate::{postgres::primitives, traits::ResolutionObservationRepository};
...
impl ResolutionObservationRepository for PgResolutionObservationRepository {
    async fn commit_scan(
        ...
        for observation in observations {
            ...
            ProjectionEntity::insert(ProjectionActiveModel {
                resolution_observation_id: Set(observation_id),
                source_checkpoint_hash: Set(observation.source_checkpoint_hash),
                status: Set(ResolutionProjectionStatus::PendingMapping),
                attempt_count: Set(0),
            ...
            .add(
                Condition::all()
                    .add(ProjectionColumn::Status.is_in([
                        ResolutionProjectionStatus::PendingMapping,
                        ResolutionProjectionStatus::Retrying,
    ...
    async fn settle(
        ...
        match settlement {
            ...
            ResolutionProjectionSettlement::Retry { retry_at, error } => {
                Self::validate_error(&error)?;
                if retry_at <= now {
                    return Err(StorageError::invariant_violation(
                        Some(QUANT_RESOLUTION_OBSERVATION_PROJECTION),
                        "retry_at must be later than PostgreSQL statement time",
                    ));
                }
                active.status = Set(ResolutionProjectionStatus::Retrying);
            ...
            ResolutionProjectionSettlement::Quarantined { retry_at, error } => {
                Self::validate_error(&error)?;
                if retry_at <= now {
                    return Err(StorageError::invariant_violation(
                        Some(QUANT_RESOLUTION_OBSERVATION_PROJECTION),
                        "quarantine retry_at must be later than PostgreSQL statement time",
                    ));
                }
            ...
            ResolutionProjectionSettlement::Failed { error } => {
                Self::validate_error(&error)?;
                active.status = Set(ResolutionProjectionStatus::Failed);
                active.canonical_fact_hash = Set(None);
                active.verified_at = Set(None);
                active.next_attempt_at = Set(None);
                active.last_error = Set(Some(error));
    ...
    async fn barrier(
        ...
        let unresolved_count = visible
            .clone()
            .filter(ProjectionColumn::Status.ne(ResolutionProjectionStatus::Verified))
            .count(&transaction)
            .await
            .map_err(StorageError::from)?;
        let quarantined_count = visible
            ...
            .filter(ProjectionColumn::Status.is_in([
                ResolutionProjectionStatus::Quarantined,
                ResolutionProjectionStatus::Failed,
            ]))
            .count(&transaction)
            .await
            .map_err(StorageError::from)?;
        ...
        Ok(ResolutionProjectionBarrier {
            ...
            verified_through: oldest_unresolved_at.unwrap_or(cutoff),
        })
    }
}
