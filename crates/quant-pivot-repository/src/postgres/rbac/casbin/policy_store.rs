...
/// Transaction-scoped owner of Casbin grouping and permission persistence.
pub struct CasbinPolicyStore<'a, C> {
    connection: &'a C,
}

impl<'a, C> CasbinPolicyStore<'a, C>
where
    C: ConnectionTrait,
{
    pub(crate) const fn new(connection: &'a C) -> Self {
        Self { connection }
    }
    ...
    pub(crate) async fn revoke_user_roles(&self, user_id: &UserId) -> Result<(), StorageError> {
        Entity::delete_many()
            .filter(Column::Ptype.eq(PTYPE_GROUPING))
            .filter(Column::V0.eq(user_id.to_string()))
            .exec(self.connection)
    ...
    /// [`Self::rebuild_role_bindings`].
    pub(crate) async fn revoke_role_bindings(
        &self,
        role_code: &RoleCode,
    ) -> Result<(), StorageError> {
        Entity::delete_many()
    ...
    pub(crate) async fn rebuild_role_bindings(
        &self,
        role_code: &RoleCode,
        user_ids: &[UserId],
    ) -> Result<(), StorageError> {
    ...
    pub(crate) async fn purge_role_code(&self, role_code: &RoleCode) -> Result<(), StorageError> {
        Entity::delete_many()
            .filter(
                Condition::any()
                    .add(
    ...
    pub(crate) async fn replace_role_policies(
        &self,
        role_code: &RoleCode,
