//! Strategy-capital equity snapshot ledger integration tests (Postgres + testcontainers).
...
async fn equity_snapshot_repo_create_latest_hwm() {
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn high_water_mark_is_monotonic_max() {
    let (pool, _container) = setup_pg().await;
    let db = pool.connection().clone();
    let repo = PgEquitySnapshotRepository::new(db);
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn drawdown_pct_is_hwm_minus_equity_over_hwm() {
    let (pool, _container) = setup_pg().await;
    let db = pool.connection().clone();
    let repo = PgEquitySnapshotRepository::new(db);
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn realized_pnl_cumulative_matches_position_ledger_sum() {
    let (pool, _container) = setup_pg().await;
    let db = pool.connection().clone();
