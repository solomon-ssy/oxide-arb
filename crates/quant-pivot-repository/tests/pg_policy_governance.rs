...
use quant_pivot_models::{
    ...
    entities::{policy_activation, policy_activation_audit, policy_activation_event_outbox},
...
async fn assert_atomic_activation_ledger(
    ...
    let audit =
        policy_activation_audit::Entity::find_by_id(committed.activation.audit_event_id.clone())
            .one(db)
            .await
            .expect("load atomic activation audit")
            .expect("activation audit exists");
    let outbox = policy_activation_event_outbox::Entity::find_by_id(
        committed.activation.audit_event_id.clone(),
    )
    .one(db)
    .await
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn outbox_failure_rolls_back_activation_snapshot_guard_and_approval_consumption() {
    let (pool, _container) = setup_pg().await;
    let db = pool.connection().clone();
    bootstrap_default_policy_bundle(
        &db,
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn rollback_records_a_new_generation_when_content_hash_matches_history() {
    let (pool, _container) = setup_pg().await;
    let db = pool.connection().clone();
    bootstrap_default_policy_bundle(
        &db,
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn concurrent_resource_activations_fail_stale_then_rebase_without_lost_updates() {
    let (pool, _container) = setup_pg().await;
    let db = pool.connection().clone();
    bootstrap_default_policy_bundle(
        &db,
    ...
    let (winner, stale) = match (recommendation_result, execution_result) {
        (Ok(winner), Err(stale)) => (
