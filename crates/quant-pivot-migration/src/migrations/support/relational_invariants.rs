...
const CONSTRAINTS: &[ConstraintSpec] = &[
    ...
    ConstraintSpec {
        name: "ck_quant_feature_parity_run_contract_hash",
        table: "quant_feature_parity_run",
        kind: ConstraintKind::Check,
        definition: "CHECK ((feature_contract_hash ~ '^blake3:[0-9a-f]{64}$'::text))",
    },
    ...
    ConstraintSpec {
        name: "uq_resolution_outcome_hash",
        table: "quant_recommendation_resolution_outcome",
        kind: ConstraintKind::Unique,
        definition: "UNIQUE (recommendation_id, outcome_hash)",
    },
    ConstraintSpec {
        name: "fk_economic_task_resolution",
        table: "quant_economic_outcome_reconciliation_task",
        kind: ConstraintKind::ForeignKey,
        definition: "FOREIGN KEY (recommendation_id, resolution_outcome_hash) REFERENCES public.quant_recommendation_resolution_outcome(recommendation_id, outcome_hash) ON UPDATE NO ACTION ON DELETE NO ACTION",
    },
    ConstraintSpec {
        name: "ck_quant_economic_outcome_task_contract",
        table: "quant_economic_outcome_reconciliation_task",
        kind: ConstraintKind::Check,
        definition: concat!(
            "CHECK ((attempt_count >= 0) AND (created_at <= updated_at) ",
            "AND ((last_error IS NULL) OR (char_length(btrim(last_error)) BETWEEN 1 AND 4096)) ",
            "AND (((attempt_count = 0) AND (replay_until IS NULL) AND (source_cutoff_at IS NULL) ",
            "AND (resolution_outcome_hash IS NULL) AND (status = 'pending'::qp_outcome_reconciliation_task_status)) ",
            "OR ((attempt_count > 0) AND (replay_until IS NOT NULL) AND (source_cutoff_at IS NOT NULL) AND (source_cutoff_at > replay_until) ",
            "AND (replay_until <= horizon_at) AND (replay_until <= updated_at) ",
            "AND (((replay_until = horizon_at) AND (resolution_outcome_hash IS NULL)) ",
            "OR ((replay_until < horizon_at) AND (resolution_outcome_hash IS NOT NULL) ",
            "AND (resolution_outcome_hash ~ '^blake3:[0-9a-f]{64}$'::text))))) ",
            "AND (((status = 'pending'::qp_outcome_reconciliation_task_status) ",
            "AND (attempt_count = 0) AND (claim_owner IS NULL) AND (lease_expires_at IS NULL) ",
            "AND (next_attempt_at IS NULL) AND (last_error IS NULL) AND (completed_at IS NULL)) ",
            "OR ((status = 'delivering'::qp_outcome_reconciliation_task_status) ",
            "AND (attempt_count > 0) AND (claim_owner IS NOT NULL) AND (lease_expires_at > updated_at) ",
            "AND (next_attempt_at IS NULL) AND (completed_at IS NULL)) ",
            "OR ((status = 'retrying'::qp_outcome_reconciliation_task_status) ",
            "AND (attempt_count > 0) AND (claim_owner IS NULL) AND (lease_expires_at IS NULL) ",
            "AND (next_attempt_at > updated_at) AND (last_error IS NOT NULL) AND (completed_at IS NULL)) ",
            "OR ((status = 'completed'::qp_outcome_reconciliation_task_status) ",
            "AND (attempt_count > 0) AND (claim_owner IS NULL) AND (lease_expires_at IS NULL) ",
    ...
    ConstraintSpec {
        name: "ck_quant_account_clean_funder_blocker_contract",
        table: "quant_account_clean_funder_blocker",
        kind: ConstraintKind::Check,
        definition: "CHECK ((role IN ('maker'::qp_account_chain_execution_role, 'self_match'::qp_account_chain_execution_role)) AND (evidence_hash ~ '^blake3:[0-9a-f]{64}$'::text))",
    },
    ConstraintSpec {
    ...
    ConstraintSpec {
        name: "ck_quant_account_pause_operation_contract",
        table: "quant_account_pause_operation",
        kind: ConstraintKind::Check,
        definition: "CHECK ((exchange_address ~ '^0x[0-9a-f]{40}$'::text) AND (requested_block >= 0) AND (((operation_kind = 'pause'::qp_account_pause_operation_kind) AND (interval_blocks >= 0) AND (effective_block = requested_block + interval_blocks)) OR ((operation_kind = 'unpause'::qp_account_pause_operation_kind) AND (interval_blocks IS NULL) AND (effective_block IS NULL))) AND (prepared_block_number >= 0) AND (prepared_block_hash ~ '^0x[0-9a-f]{64}$'::text) AND (calldata_hash ~ '^0x[0-9a-f]{64}$'::text) AND (deployment_digest ~ '^blake3:[0-9a-f]{64}$'::text) AND (octet_length(signed_envelope) > 0) AND (signed_envelope_hash ~ '^blake3:[0-9a-f]{64}$'::text) AND (((submission_kind = 'direct_eoa'::qp_settlement_submission_kind) AND (transaction_hash IS NOT NULL) AND (relayer_transaction_id IS NULL) AND (gas_limit IS NOT NULL)) OR ((submission_kind = 'relayer'::qp_settlement_submission_kind) AND (transaction_hash IS NULL))) AND (((confirmation_block_number IS NULL) AND (confirmation_block_hash IS NULL) AND (confirmation_transaction_hash IS NULL) AND (confirmation_log_index IS NULL)) OR ((confirmation_block_number >= 0) AND (confirmation_block_hash ~ '^0x[0-9a-f]{64}$'::text) AND (confirmation_transaction_hash ~ '^0x[0-9a-f]{64}$'::text) AND (confirmation_log_index >= 0))) AND (((state = 'prepared'::qp_account_pause_operation_state) AND (dispatched_at IS NULL) AND (confirmed_at IS NULL) AND (confirmation_block_number IS NULL)) OR ((state IN ('dispatched'::qp_account_pause_operation_state, 'ambiguous'::qp_account_pause_operation_state)) AND (dispatched_at IS NOT NULL) AND (confirmed_at IS NULL) AND (confirmation_block_number IS NULL)) OR ((state = 'confirmed'::qp_account_pause_operation_state) AND (dispatched_at IS NOT NULL) AND (confirmed_at IS NOT NULL) AND (confirmation_block_number IS NOT NULL)) OR ((state = 'failed'::qp_account_pause_operation_state) AND (failure_detail IS NOT NULL))))",
    },
    ConstraintSpec {
        ...
        definition: concat!(
            ...
            "AND (((status = 'open'::qp_account_recovery_incident_status) ",
            "AND seal_hash IS NULL AND sealed_by IS NULL AND sealed_at IS NULL) ",
            "OR ((status = 'reconciling'::qp_account_recovery_incident_status) AND ",
            "(((seal_hash IS NULL) AND (sealed_by IS NULL) AND (sealed_at IS NULL)) OR ",
            "((seal_hash ~ '^blake3:[0-9a-f]{64}$'::text) AND sealed_by IS NOT NULL ",
    ...
    ConstraintSpec {
        name: "ck_quant_account_recovery_manifest_contract",
        table: "quant_account_recovery_manifest",
        kind: ConstraintKind::Check,
        definition: concat!(
            "CHECK ((attempt_no > 0) AND (finalized_block_number >= 0) ",
            "AND (finalized_block_hash ~ '^0x[0-9a-f]{64}$'::text) ",
            "AND (evidence_hash ~ '^blake3:[0-9a-f]{64}$'::text) ",
            "AND (jsonb_typeof(input_json) = 'object') ",
            "AND ((input_json ->> 'finalized_block_number')::bigint = finalized_block_number) ",
            "AND (input_json ->> 'finalized_block_hash' = finalized_block_hash) ",
            "AND (jsonb_typeof(assessment_json) = 'object') ",
            "AND (assessment_json ?& ARRAY['allocations', 'created_lots', 'mismatches', 'evidence_hash']) ",
            "AND (assessment_json - ARRAY['allocations', 'created_lots', 'mismatches', 'evidence_hash'] = '{}'::jsonb) ",
            "AND (assessment_json ->> 'evidence_hash' = evidence_hash) ",
            "AND (jsonb_typeof(assessment_json -> 'mismatches') = 'array') ",
            "AND (jsonb_typeof(created_lots_json) = 'array') ",
            "AND (converged = (jsonb_array_length(assessment_json -> 'mismatches') = 0)))",
        ),
    },
    ConstraintSpec {
        name: "ck_recommendation_economic_outcome_contract",
        table: "quant_recommendation_economic_outcome",
        kind: ConstraintKind::Check,
        definition: concat!(
            "CHECK ((decision_at < horizon_at) AND (source_available_until <= available_at) AND (available_at <= created_at) ",
            "AND (char_length(replay_kernel_version) BETWEEN 1 AND 128) ",
            "AND (replay_kernel_version = btrim(replay_kernel_version)) ",
            "AND (evidence_hash ~ '^blake3:[0-9a-f]{64}$'::text) ",
            "AND (jsonb_typeof(payload_json) = 'object') ",
            "AND (payload_json ?& ARRAY['detail', 'amounts', 'evidence']) ",
            "AND (payload_json - ARRAY['detail', 'amounts', 'evidence'] = '{}'::jsonb) ",
            "AND (payload_json -> 'detail' ->> 'kind' = state::text) ",
            "AND (jsonb_typeof(payload_json -> 'amounts') = 'object') ",
            "AND (jsonb_typeof(payload_json -> 'evidence') = 'object') ",
            "AND ((state = 'censored'::qp_recommendation_economic_outcome_state) OR ",
            "((payload_json -> 'amounts' -> 'net_pnl_usd') <> 'null'::jsonb)) ",
            "AND ((state <> 'horizon_liquidated'::qp_recommendation_economic_outcome_state) OR ",
            "((payload_json -> 'evidence' ->> 'exit_evidence_kind' = 'full_bid_ladder') ",
            "AND ((payload_json -> 'evidence' ->> 'full_l2_covered')::boolean))))",
        ),
    },
    ConstraintSpec {
        name: "uq_route_economic_health_cutoff",
        table: "quant_route_economic_health",
        kind: ConstraintKind::Unique,
        definition: "UNIQUE (route_identity_hash, research_profile_artifact_id, assessed_through)",
    },
    ConstraintSpec {
        name: "ck_route_economic_health_contract",
        table: "quant_route_economic_health",
        kind: ConstraintKind::Check,
        definition: concat!(
            "CHECK ((due_observation_count >= 0) ",
            "AND (usable_observation_count >= 0) ",
            "AND (usable_observation_count <= due_observation_count) ",
            "AND (comparison_minimum_observations > 0) ",
            "AND (coverage >= 0) AND (coverage <= 1) ",
            "AND (minimum_coverage > 0) AND (minimum_coverage <= 1) ",
            "AND (minimum_effect_bps > 0) ",
            "AND (confidence > 0) AND (confidence <= 1) ",
            "AND ((window_start IS NULL) = (due_observation_count = 0)) ",
            "AND ((window_start IS NULL) OR (window_start < assessed_through)) ",
            "AND (assessed_through <= available_at) AND (available_at <= created_at) ",
            "AND (route_identity_hash ~ '^blake3:[0-9a-f]{64}$'::text) ",
            "AND (feedback_policy_hash ~ '^blake3:[0-9a-f]{64}$'::text) ",
            "AND (evidence_hash ~ '^blake3:[0-9a-f]{64}$'::text) ",
            "AND (jsonb_typeof(evidence_json) = 'object') ",
            "AND (evidence_json ?& ARRAY['observation_hash', 'uniqueness_weight_hash', 'methodology_version']) ",
            "AND (evidence_json - ARRAY['observation_hash', 'uniqueness_weight_hash', 'methodology_version'] = '{}'::jsonb) ",
            "AND ((evidence_json ->> 'observation_hash') ~ '^blake3:[0-9a-f]{64}$'::text) ",
            "AND (char_length(btrim(evidence_json ->> 'methodology_version')) BETWEEN 1 AND 128) ",
            "AND (((state IN ('insufficient_evidence'::qp_route_economic_health_state, 'data_incomplete'::qp_route_economic_health_state)) ",
            "AND (effective_sample_size IS NULL) AND (weighted_mean_return_bps IS NULL) ",
            "AND (lower_confidence_return_bps IS NULL) AND ((evidence_json -> 'uniqueness_weight_hash') = 'null'::jsonb)) ",
            "OR ((state IN ('healthy'::qp_route_economic_health_state, 'degraded'::qp_route_economic_health_state)) ",
            "AND (effective_sample_size > 0) AND (weighted_mean_return_bps IS NOT NULL) ",
            "AND (lower_confidence_return_bps IS NOT NULL) ",
            "AND ((evidence_json ->> 'uniqueness_weight_hash') ~ '^blake3:[0-9a-f]{64}$'::text))) ",
            "AND ((state <> 'insufficient_evidence'::qp_route_economic_health_state) ",
            "OR (due_observation_count < comparison_minimum_observations)) ",
            "AND ((state <> 'data_incomplete'::qp_route_economic_health_state) ",
            "OR (coverage < minimum_coverage) OR (usable_observation_count < comparison_minimum_observations)) ",
            "AND ((state <> 'healthy'::qp_route_economic_health_state) ",
            "OR ((usable_observation_count >= comparison_minimum_observations) ",
            "AND (coverage >= minimum_coverage) AND (lower_confidence_return_bps >= minimum_effect_bps))) ",
            "AND ((state <> 'degraded'::qp_route_economic_health_state) ",
            "OR ((usable_observation_count >= comparison_minimum_observations) ",
            "AND (coverage >= minimum_coverage) AND (lower_confidence_return_bps < minimum_effect_bps))))",
        ),
    },
