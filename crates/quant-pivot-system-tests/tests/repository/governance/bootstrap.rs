//! Durable cold-start FSM persistence system contract for the typed policy boot bundle.

use chrono::Duration;
use quant_pivot_models::{
    domain::governance::ActivateBootstrapState,
    entities::system_bootstrap_transition::{Column, Entity},
...
pub async fn bootstrap_transitions_are_monotonic_restart_safe_and_policy_bundle_bound() {
    ...
    let transitions = Entity::find()
        .filter(Column::BootstrapContractVersion.eq(active_restart.bootstrap_contract_version))
        .order_by_asc(Column::StateRevision)
        .all(&db)
        .await
