//! Bitemporal Gamma catalog ledger persistence system contracts.
...
use quant_pivot_models::{
    ...
    entities::{
        catalog_event_change::Entity as CatalogEventChangeEntity,
        catalog_event_object::Entity as CatalogEventObjectEntity,
        catalog_market_change::Entity as CatalogMarketChangeEntity,
        catalog_market_object::Entity as CatalogMarketObjectEntity,
        catalog_sync_batch::Entity as CatalogSyncBatchEntity, event::Entity as EventEntity,
        market::Entity,
    },
    enums::{
        catalog::{
...
pub async fn projection_upsert_updates_status() {
    ...
    let filtered_projection = Entity::find_by_id(MarketId::new("0xcatalog-ledger"))
        .one(db)
        .await
    ...
    let active_projection = Entity::find_by_id(MarketId::new("0xcatalog-ledger"))
        .one(db)
        .await
...
async fn catalog_row_counts(db: &DatabaseConnection) -> CatalogRowCounts {
    CatalogRowCounts {
        batches: CatalogSyncBatchEntity::find()
            .count(db)
            .await
            .expect("count catalog batches"),
        event_objects: CatalogEventObjectEntity::find()
            .count(db)
            .await
            .expect("count catalog event objects"),
        event_changes: CatalogEventChangeEntity::find()
            .count(db)
            .await
            .expect("count catalog event changes"),
        market_objects: CatalogMarketObjectEntity::find()
            .count(db)
            .await
            .expect("count catalog market objects"),
        market_changes: CatalogMarketChangeEntity::find()
            .count(db)
            .await
            .expect("count catalog market changes"),
        events: EventEntity::find()
            .count(db)
            .await
...
fn snapshot_revisions(snapshots: &[CatalogSnapshotInfo]) -> BTreeSet<&str> {
    snapshots
        .iter()
        .flat_map(|snapshot| {
            iter::once(&snapshot.market)
                .chain(snapshot.event_markets.iter())
                .map(|market| market.payload["revision"].as_str())
                .chain(iter::once(snapshot.event.payload["revision"].as_str()))
        })
        .flatten()
        .collect()
}
