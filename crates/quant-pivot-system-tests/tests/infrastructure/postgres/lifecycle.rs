//! Fresh-install and lifecycle-lease `PostgreSQL` system contracts.
...
pub async fn lifecycle_lease_is_exclusive_and_cancels_after_session_loss() {
    let config = PostgresConfig {
        min_connections: 1,
        max_connections: 2,
        verify_session_params: false,
        ..create_empty_postgres("lifecycle_lease").await
    };
...
pub async fn reset_rejects_unknown_sessions_and_never_forces_them_closed() {
    let base_config = fresh_postgres_config("lifecycle_reset");
