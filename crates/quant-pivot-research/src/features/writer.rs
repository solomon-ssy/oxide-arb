...
use quant_pivot_models::{
    ...
    types::{ContentHash, DecisionPolicySnapshotId, FeatureVectorId, MarketId, TokenId},
};
use serde::Serialize;
...
struct FeatureEventAudit<'a> {
    ...
    feature_hash: &'a ContentHash,
    decision_capture_hash: &'a ContentHash,
    feature_name: &'a str,
    cell_state: i8,
...
fn project_event(
    ...
    let audit = FeatureEventAudit {
        ...
        feature_hash: &context.persisted.feature_hash,
    ...
    let audit_fingerprint = ResearchHasher::canonical(&audit)?.to_string();
    Ok(QuantFeatureEventRow {
        ...
        feature_schema_version: context.schema_version,
        feature_schema_hash: context.schema_hash.to_string(),
        feature_hash: context.persisted.feature_hash.to_string(),
        decision_capture_hash: decision_capture_hash.to_string(),
...
fn raw_value_text(value: &FeatureValue) -> String {
    match value {
        ...
        FeatureValue::Category(value) => value.to_string(),
    }
}
