//! Versioned, hot-reloadable runtime configuration (`schema_version = 4`).
...
mod tests {
    ...
    fn schema_version_is_four() {
        assert_eq!(RuntimeConfig::default().schema_version.get(), 4);
        assert_eq!(RUNTIME_CONFIG_SCHEMA_VERSION.get(), 4);
    }
