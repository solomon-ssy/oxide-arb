...
use crate::{
    ...
    enums::{
        common::Side,
        execution::{
            ApprovalInvalidation, ExecutionOrderPhase, ExitReason, ExitState, OrderTypeKind,
            VenueOrderStatus, VenueTradeStatus,
        },
        quant::{
            AuthorizationKind, ExecutionOrderState, OrderIntentStatus, RecommendationReportStatus,
        },
    },
    types::{
        AuthorizationEvidence, ContentHash, DecisionPolicySnapshotId, EntryConditionInstanceId,
        EntryOrderSpec, EvmTransactionHash, ExecutionAccountId, ExecutionOrderId,
        ExecutionTradeRefId, ExecutionTransactionRefId, ExitPolicySpec, ExitReinferenceObservation,
        MarketId, ModelVersionId, OrderId, OrderIntentId, PreparedVenueOrder, Price,
        RecommendationId, ResearchProfileArtifactId, ResearchProfileRef, ScaleOutState, Shares,
        TokenId, Usd, VenueTradeId,
    },
};
...
impl From<Model> for OrderIntentInfo {
    fn from(model: Model) -> Self {
        Self {
            ...
            status: model.status,
            authorization_kind: model.authorization_kind,
            authorization_evidence: model.authorization_evidence,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuthorizeOrderIntent {
