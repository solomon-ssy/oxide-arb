...
impl DeployValidationRuleContract {
    pub(super) const ALL: &'static [Self] = &[
        ...
        Self::new(
            ...
            "Always",
            "All four values must be greater than zero; page_size must be no greater than the Gamma /events/keyset limit of 500; max_keyset_requests must be no less than max_keyset_pages.",
        ),
        Self::new(
            "deploy.trade-tape.work-budget",
            &[
                "market_data.trade_tape_on_chain.max_blocks_per_tick",
                "market_data.trade_tape_on_chain.max_blocks_per_request",
                "market_data.trade_tape_on_chain.batch_size",
                "market_data.trade_tape_on_chain.reconciliation_match_window_ms",
                "market_data.trade_tape_on_chain.reconciliation_terminal_age_secs",
                "market_data.trade_tape_on_chain.reconciliation_max_rows",
            ],
            "Always",
            "Block, batch, matching-window, terminal-age, and reconciliation-row budgets must each be greater than zero; reconciliation_max_rows must be no greater than the native SQL result budget of 1000000 rows.",
        ),
        Self::new(
            "deploy.trade-tape.lookback",
            &[
                "market_data.trade_tape_on_chain.reconciliation_lookback_secs",
                "market_data.trade_tape_on_chain.reconciliation_terminal_age_secs",
            ],
            "Always",
            "reconciliation_lookback_secs must be greater than reconciliation_terminal_age_secs.",
        ),
        Self::new(
        ...
        Self::new(
            ...
            "Always",
            "The batch size, flush interval, and concurrent insert limit must each be greater than zero.",
        ),
        Self::new(
        ...
        Self::new(
            ...
            "When runtime mode permits order submission and wallet_kind is proxy, gnosis_safe, or deposit_wallet",
