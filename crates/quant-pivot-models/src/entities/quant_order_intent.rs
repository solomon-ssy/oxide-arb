...
use super::{
    decision_policy_snapshot, quant_capital_allocation, quant_clob_trade_observation,
    quant_entry_condition_instance, quant_execution_account, quant_execution_attempt_outcome,
    quant_execution_attempt_reconciliation_task, quant_execution_order, quant_model_version,
    quant_recommendation, quant_settlement_inventory_lot, quant_strategy_position_lot,
    research_profile_artifact,
};
use crate::{
    enums::{
        execution::{ExitReason, ExitState},
        quant::{AuthorizationKind, OrderIntentStatus},
    },
    types::{
        AuthorizationEvidence, ContentHash, DecisionPolicySnapshotId, EntryConditionInstanceId,
        EntryOrderSpec, ExecutionAccountId, ExitPolicySpec, ExitReinferenceObservation,
        ModelVersionId, OrderIntentId, Price, RecommendationId, ResearchProfileArtifactId,
        ScaleOutState,
    },
};
...
pub struct Model {
    ...
    pub position: HasOne<quant_strategy_position_lot::Entity>,
